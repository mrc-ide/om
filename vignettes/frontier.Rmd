...
frontier solution and moves stepwise up the frontier. In some cases we may want 
to specify the starting solution and work up and down the frontier from that
point. This is done by indexing the startiung solution in the `frontier()`. This
starting solution will always be included in the output, even if it does not
lie on the frontier.
...
points(frontier_solution$impact[start_index] ~ frontier_solution$cost[start_index], col = "turquoise", pch = 19)
legend(
  "bottomright",
...
We may want to apply other filtering moves when traversing up or fown the 
frontier. We can define custom filter functions for this:
...
frontier_threshold_solution_filter <- frontier(example, threshold = 1,
                                        down_filter = down_filter, up_filter = up_filter,
                                        start_index = 3)
plot(
  example$impact ~ example$cost,
  pch = 19,
